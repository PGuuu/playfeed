...
const OFFICIAL_GAME_IDS = [
  ...
  'crossy-chicken', 'softserve', 'last-rain-bus', 'sand-canvas', 'neon-last-stand',
];
...
async function syncOfficialGames(user, body) {
  ...
  const rows = OFFICIAL_GAME_IDS.map((id, index) => {
    ...
    return {
      ...
      suggested_id: id,
      api_version: 1,
      game_version: id === 'neon-last-stand' ? 'official-1.2.0' : 'official-1.0.0',
      ...
      duration: Math.min(60, Math.max(20, Number(game.duration) || 45)),
